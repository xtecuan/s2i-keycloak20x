...
#'--optimized'
echo "kc.sh start   --db-url-host $KC_DB_URL --db-username $KC_DB_USERNAME --db-password $KC_DB_PASSWORD --hostname-strict false --https-key-store-file $KC_KEYSTORE --hostname $KC_HOSTNAME" > run.sh
chmod a+x run.sh
exec ./run.sh